...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'gift_redemptions' AND column_name = 'redemption_code') THEN
        ...
        -- 为现有记录生成兑换码
        UPDATE gift_redemptions SET redemption_code = 'SGZ' || UPPER(SUBSTRING(MD5(id::text || created_at::text) FROM 1 FOR 8)) WHERE redemption_code IS NULL;
        -- 设置为 NOT NULL
        ALTER TABLE gift_redemptions ALTER COLUMN redemption_code SET NOT NULL;
    END IF;
...
BEGIN
    ...
    -- 标记为已完成，但不在此处发放奖励（避免重复），奖励通过 claim_task_reward 发放
