...
-- 2. 创建 complete_task 函数 - 完成任务（完全修复版）
...
-- 3. 创建 redeem_gift 函数 - 兑换礼品（完全修复版）
...
BEGIN
  ...
  -- 扣除光币
  UPDATE public.profiles
  SET coins = coins - v_price::INTEGER
  WHERE id = user_uuid;
...
-- 7. 创建 handle_new_user 函数 - 用户注册触发器（修复版）
