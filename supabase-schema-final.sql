...
-- 2. 删除可能有问题的 gift_redemptions 表并重新创建
DROP TABLE IF EXISTS gift_redemptions CASCADE;
...
DECLARE
    result JSON;
    -- perform_check_in 仅记录签到（streak / check_ins），不在此处直接发放光币/记录交易。
...
BEGIN
    ...
    -- 不在此处发放奖励。任务奖励应通过 complete_task / claim_task_reward 发放，以避免重复领取。
